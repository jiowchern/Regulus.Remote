// --------------------------------------------------------------------------------------------------------------------
// <copyright file="CommandSender.cs" company="">
//   
// </copyright>
// <summary>
//   Defines the CommandSender type.
// </summary>
// --------------------------------------------------------------------------------------------------------------------
...
namespace Regulus.Utility
{
	public class CommandSender
	{
		...
		{
			this._CommandParamParsers = parsers;
		}

		public void AddInvoker(string name, object invoker, Type type)
		{
			this._CommandInvoker.Add(new CommandInvoker
			{
				Name = name, 
				Handler = invoker, 
				Type = type
		...
		{
			this._CommandInvoker.RemoveAll(invoker => invoker.Name == name);
		}
		...
		public void Send(string command_feedstock)
		{
			...
			if (firstGroup != null)
			{
				...
				if (commandAndArgs.Count() > 1)
				{
					...
					var first = (from ch in this._CommandInvoker
					             where ch.Name == name
					             select new
					             {
						             handler = ch.Handler, 
					...
					if (first != null)
					{
						...
						var args = CommandSender._GetParams(first.type, command, commandAndArgs[3], this._CommandParamParsers);
						try
						{
							first.type.InvokeMember(command, BindingFlags.Public | BindingFlags.Instance | BindingFlags.InvokeMethod, null, 
								first.handler, args);
						}
						catch (Exception e)
						{
