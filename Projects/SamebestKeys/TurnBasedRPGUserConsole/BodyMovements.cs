...
namespace Regulus.Project.SamebestKeysUserConsole.BotStage
{
    class BodyMovements : Regulus.Game.IStage<StatusBotController>
    {
        ...
        void PlayerProvider_Supply(SamebestKeys.IPlayer obj)
        {
            _ToMove = new Regulus.Utility.TimeCounter();
            var actionStatus = Enum.GetValues(typeof(Regulus.Project.SamebestKeys.ActionStatue));
            if (actionStatus.Length > 0)
            {
                var a = (Regulus.Project.SamebestKeys.ActionStatue)Regulus.Utility.Random.Next(0, actionStatus.Length - 1);
                obj.BodyMovements(a);
